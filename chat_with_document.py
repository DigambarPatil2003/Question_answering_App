...
# Lets Create the function to create the embeddings and store it into the Chroma
# def create_embeddings(chunks):
#     from transformers import AutoTokenizer, TFAutoModel
#     import tensorflow as tf

#     model_name = "sentence-transformers/all-MiniLM-L6-v2"
#     model_dir = "./local_model"  # Directory where you want to save the model
...
#         inputs = tokenizer(chunk_text, return_tensors="tf", truncation=True, padding=True)
#         outputs = model(**inputs)

#         # Mean pooling to get a single embedding for the chunk
#         embedding = tf.reduce_mean(outputs.last_hidden_state, axis=1).numpy()
#         embeddings.append(embedding.flatten())  # Flatten the embedding

#     # Convert embeddings to a format suitable for Chroma
#     embeddings = [embedding.tolist() for embedding in embeddings]  # Convert to lists for Chroma
...
if __name__ == '__main__':
    ...
    st.subheader('LLM Auestion Answering Application 🤖')
...
q = st.text_input('Enter your Question on document: ')
if q:
